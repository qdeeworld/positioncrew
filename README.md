...
The repository also includes a deterministic TermiX A2A host (`npm run runtime:termix`). It accepts only an agent-scoped runtime token, refuses to start when wallet signing material is present, stops before the token expires, deduplicates overlapping inbox polls, answers service inquiries from fixed provider contracts, and escalates delivery, challenge, and operator-case threads instead of auto-acting. All four active provider identities belong to the dedicated production wallet `0xADd748C416E8A7efd7d65D18Abb121dea268ddF9`. Each service has a separate runtime, scoped token, state directory, and hourly renewal timer; the unprivileged pollers never receive the owner key. Three completed automatic rotations are recorded in [`evidence/termix-runtime-rotations.mainnet.json`](evidence/termix-runtime-rotations.mainnet.json). They are discrete host-observed renewal events, not evidence of continuous uptime, availability between observations, or future renewal success. Current identity and listing records are in `evidence/termix-dedicated-identities.mainnet.json` and `evidence/termix-dedicated-listings.mainnet.json`. The original personal-wallet records remain unchanged as historical evidence. Lending Rescue remains available in the compatibility `dedicatedFlagship` field and is also one of the four active providers.
...
The independent read-only VPS health monitor runs every five minutes through November 5, 2026 UTC and stops on November 6. This monitoring window does not extend the separate Altana testnet signing session, which expires on September 24. Historical submission and study artifacts retain their original release dates; subsequent maintenance does not turn new runtime identities into pre-deadline evidence.

### Accepted Lending order fulfillment

`fulfill-termix-lending` supports unattended delivery for explicitly authorized, already-accepted Lending Rescue orders. It reads buyer requirements from the authenticated order scope, generates a fresh Venus assessment through the existing provider, verifies the uploaded artifact, and submits only the guarded `submitDelivery` transaction. It does not accept new orders or release buyer escrow. The provider-order observer remains an alerting service.

The worker requires a protected policy file binding one order ID, on-chain order ID, client account, exact scope hash, expiry, and maximum gas liability (at most 0.000034 BNB). The current route admits only the dedicated Lending listing at 5 USDC. Scope input accepts the strict structured Lending request JSON or the documented fully specified prose used in the order test; unsupported prose requires clarification. No model guesses constraints. The example units in `deploy/systemd/positioncrew-termix-lending-fulfill.*` use systemd credentials and a separate state directory; install both compiled fulfillment tools root-owned before enabling. Provision `positioncrew-fulfill` as a system user and provide `/etc/positioncrew-runtime/lending-delivery-policy.json` root-owned 0600. Rotate policy only for separately authorized work. Disable the timer when the bounded order completes or the policy expires.

Signed delivery transactions are journaled before broadcast and reconciled before further work. An expired unresolved signed transaction or a revert requires operator reconciliation; retries never generate a replacement payment. The local and production order schemas support the current nested marketplace DTO while rejecting conflicting identity or amount fields. These changes address the September 10 paid-order observations; broad autonomous acceptance and arbitrary natural-language intake remain outside this rollout.

On LXC hosts with a global `zzz-lxc-service.conf` that clears credentials, install the provided per-unit `positioncrew-termix-lending-fulfill.service.d/zzzz-load-credential.conf` under `/etc/systemd/system/`, then daemon-reload and inspect the effective `LoadCredential`, `NoNewPrivileges`, and `TasksMax`. The worker and its Node preparation subprocess require `TasksMax=64`. Delivery preparation sends artifact IDs and validates the returned pending manifest contains exactly the verified report for the order. It keeps the file SHA256 separate from the API-computed manifest hash, then checks the transaction call data against that manifest hash. Pending artifacts are not exposed by the order attachment list until delivery is indexed.

The [current TermiX order reference](https://docs.termix.ai/api-reference/orders) assigns manifest construction to the backend when `artifactIds` is supplied. PositionCrew trusts the official authenticated API for that serialization; it does not claim to independently derive the manifest hash. It independently checks report bytes against their SHA256, validates the returned manifest contains the exact report/order, and guards transaction recipient, method, order, value and returned hash. New protected checkpoints mark this artifactIds flow and reuse the verified pending intent on retries; legacy checkpoints are rebuilt.
