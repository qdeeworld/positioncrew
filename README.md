...
On LXC hosts with a global `zzz-lxc-service.conf` that clears credentials, install the provided per-unit `positioncrew-termix-lending-fulfill.service.d/zzzz-load-credential.conf` under `/etc/systemd/system/`, then daemon-reload and inspect the effective `LoadCredential`, `NoNewPrivileges`, and `TasksMax`. The worker and its Node preparation subprocess require `TasksMax=64`. Delivery preparation includes the verified artifact ID alongside its content hash and refuses to hand off a submit intent unless the report remains attached in the marketplace API.
