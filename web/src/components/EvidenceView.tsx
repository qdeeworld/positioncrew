          ...
          {publishedFounderAdvantage ? (
            <div className="claim-warning published">
              <BadgeCheck size={16} aria-hidden="true" />
                  <span><strong>Founder-operated comparison published.</strong>{publishedFounderAdvantage.exactOutputParityCount}/3 tasks record exact canonical output parity and {publishedFounderAdvantage.recordedSpeedAdvantageCount}/3 record lower agent time. The selected arms are qualified <code>E3_SERVER_PERSISTED</code> fresh PositionCrew server-persisted $0.00, no-wallet historical-fixture hires with unique D1 records and public receipts. This does not establish paid commerce or an external buyer. Quality evidence here is exact canonical output parity, not a numeric rating. No separate numeric quality score was assigned. The comparison is non-independent and non-blind. <a href={publishedFounderAdvantage.reportUrl} target="_blank" rel="noreferrer">Inspect the bounded report.</a></span>
            </div>
