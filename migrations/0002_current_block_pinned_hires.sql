CREATE TABLE fresh_marketplace_hires_next (
  ...
  CHECK (evidence_mode = 'HISTORICAL_FIXTURE' OR (benchmark_slug = 'lending-rescue' AND provider_slug = 'lending-rescue' AND service = 'LENDING_RESCUE')),
  CHECK (
...
CREATE TABLE fresh_marketplace_rate_limits (
  key_hash TEXT PRIMARY KEY NOT NULL CHECK (length(key_hash) = 71 AND substr(key_hash, 1, 7) = 'sha256:'),
