...
const SHADOW_GRID_GUARD_STATEMENTS = [
  "CREATE TRIGGER IF NOT EXISTS shadow_grid_events_no_update BEFORE UPDATE ON shadow_grid_events BEGIN SELECT RAISE(ABORT, 'shadow_grid_events is append-only'); END",
  "CREATE TRIGGER IF NOT EXISTS shadow_grid_events_no_delete BEFORE DELETE ON shadow_grid_events BEGIN SELECT RAISE(ABORT, 'shadow_grid_events is append-only'); END",
  [
    "CREATE TRIGGER IF NOT EXISTS shadow_grid_events_validate_reference BEFORE INSERT ON shadow_grid_events BEGIN",
    "SELECT CASE WHEN NOT EXISTS (",
    "SELECT 1 FROM fresh_marketplace_hires h JOIN fresh_marketplace_jobs j ON j.hire_id = h.hire_id",
    ...
    ") THEN RAISE(ABORT, 'shadow grid event must reference an exact completed current bounded-grid receipt') END; END",
  ].join(" "),
  [
    "CREATE TRIGGER IF NOT EXISTS shadow_grid_events_validate_chain BEFORE INSERT ON shadow_grid_events BEGIN",
    ...
    "SELECT CASE WHEN EXISTS (SELECT 1 FROM shadow_grid_events first_event WHERE first_event.run_id = NEW.run_id AND first_event.event_sequence = 0 AND (",
