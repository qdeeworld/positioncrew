...
export async function auditionBrainOnBnbGrid(
  ...
  try {
    ...
    const checks: BrainOnBnbGridComparison["checks"] = [
      ...
      { code: "EXACT_POOL_AND_PAIR", status: exactPool && exactPair ? "PASS" : "FAIL", detail: exactPool && exactPair ? `The provider measured the requested PancakeSwap ${request.baseAsset.symbol}/${request.quoteAsset.symbol} pool.` : "The provider result did not bind the requested pool and pair." },
