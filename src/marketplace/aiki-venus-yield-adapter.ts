...
async function readPinnedSupplyRates(
  request: YieldOptimizationRequest,
  fetchImpl: typeof fetch,
  rpcUrl: string,
): Promise<Map<string, bigint>> {
  const blockTag = `0x${pinnedBlock(request).toString(16)}`;
  const data = encodeFunctionData({ abi: VTOKEN_ABI, functionName: "supplyRatePerBlock" });
  const entries = await Promise.all(request.opportunities.map(async (opportunity, index) => {
    const response = await fetchImpl(rpcUrl, {
      ...
      body: JSON.stringify({
        ...
        id: index + 1,
        method: "eth_call",
        params: [{ to: opportunity.vaultOrMarket, data }, blockTag],
      }),
    ...
    return [opportunity.vaultOrMarket.toLowerCase(), rate] as const;
  }));
...
export async function auditionAiKiVenusYield(
  ...
  try {
    ...
    const [response, pinnedRates] = await Promise.all([
      fetchImpl(url, {
        headers: { accept: "application/json" },
        signal: AbortSignal.timeout(2_500),
      }),
      readPinnedSupplyRates(request, fetchImpl, options.rpcUrl ?? DEFAULT_BSC_RPC),
    ]);
    ...
    const normalizedDeliverable = recommendedOpportunity
      ? createYieldOptimizationDeliverable({ ...request, opportunities: [recommendedOpportunity] }, now)
      : undefined;
